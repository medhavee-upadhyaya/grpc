...
TEST_F(PartyTest, Test17SpawnedPromises) {
      ...
      [p16_polled_once = false, &p16_waker, &p16_polled,
       &execution_order]() mutable -> Poll<Empty> {
        if (!p16_polled_once) {
          ...
          absl::StrAppend(&execution_order, "2");
          p16_waker = GetContext<Activity>()->MakeOwningWaker();
          absl::StrAppend(&execution_order, "3");
          p16_polled.Notify();
          return Pending{};
        }
  ...
  EXPECT_EQ(execution_order, "123W4567");
  VLOG(2) << "17th promise completed successfully after slot was freed.";
}

TEST_F(PartyTest, Test17SpawnedPromisesSpawnedFromWithinParty) {
  const RefCountedPtr<Party> party = MakeParty();
  std::string execution_order;
  ...
  // Step 4 & 6: Setup 17th promise notification, and spawn a 16th promise that
  // spawns the 17th promise on its first poll.
  Notification p17_completed;
  Waker p16_waker;
  Notification p16_polled;
  Notification p16_completed;
  ...
  party->Spawn(
      "p16",
      [p16_polled_once = false, &p16_waker, &p16_polled, party, &p17_completed,
       &execution_order]() mutable -> Poll<Empty> {
        if (!p16_polled_once) {
          p16_polled_once = true;
          absl::StrAppend(&execution_order, "2");
          ...
          // Spawn the 17th promise from within the 16th promise
          party->Spawn("p17", Immediate(Empty{}),
                       [&execution_order, &p17_completed](const Empty) {
                         absl::StrAppend(&execution_order, "6");
                         p17_completed.Notify();
                       });
          absl::StrAppend(&execution_order, "3");
      ...
      [&execution_order, &p16_completed](const Empty) {
        absl::StrAppend(&execution_order, "5");
        p16_completed.Notify();
      });

  // Step 5: Synchronize to ensure the waker has been created and p17 has been
  // spawned.
  p16_polled.WaitForNotification();
  ...
  // Step 8: Wake up the 16th promise and wait for it to complete, freeing the
  // 16th slot.
  absl::StrAppend(&execution_order, "W");
  p16_waker.Wakeup();
  p16_completed.WaitForNotification();
